...
// with that bit set, so it can never compile (which also makes tt-llk's own six-entry
// _calculate_sigmoid_ dead code -- see ckernel_sfpu_sigmoid_appx.h). The restriction is sfpi's,
// not the hardware's: the mod assembles and is verified on silicon.
//
