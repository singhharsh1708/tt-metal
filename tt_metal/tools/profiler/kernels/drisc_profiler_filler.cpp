...
void kernel_main() {
    ...
    *phase = kPhaseInit;
    ...
    // Per core: sum of the five tails at its last SCAN. Tails are monotonic, so the delta is exactly
    // the words the core produced in one service interval -- the growth term the deferral guard needs.
    ...
    uint32_t svc_max = 0;
    // kSvcInstr tail hunt: per-phase worst case with the timer_lo it happened at, so a stall in the
    // stall CSV (absolute cycles) can be attributed to the phase whose spike immediately precedes it.
    uint32_t max_bar = 0, max_bar_at = 0;    // ship barrier (DMA completion / write flush)
    uint32_t max_rdw = 0, max_rdw_at = 0;    // gather read-barrier wait
    uint32_t max_cvw = 0, max_cvw_at = 0;    // sweep-start CV read + barrier
    uint32_t max_pump = 0, max_pump_at = 0;  // one drain_pump pass
    ...
    static uint32_t slot_live[kNStage];
    static uint8_t slot_hb[kNStage];
    ...
    // Ship-threshold arming: batching must never hold pre-burst trickle across a burst onset (the ring
    // fills 0->full in about the filler's detection latency, so a pre-loaded ring stalls its producer),
    // and level can't distinguish one-shot trickle from a light workload's steady sub-threshold lanes.
    // Growth persistence can: defer only after kBatchArmSweeps consecutive growing sweeps, and flush
    // everything once the grid has been dead for kFlushQuietSweeps (the NoC is idle then anyway).
    ...
    uint64_t spool_oldest = 0;      // when the spool last went non-empty; 0 = empty (freshness deadline)
    uint32_t fresh_tick = 0;
    // ~50 ms at 1.35 GHz: longer than any knee-regime burst survives without ending or hitting 5/8.
    ...
    auto drain_pump = [&]() -> bool {
        if constexpr (!kSpool) {
            ...
            const uint64_t t_dp0 = kSvcInstr != 0 ? get_timestamp() : 0;
            const auto dp_track = [&](bool moved) {
                if constexpr (kSvcInstr != 0) {
                    const uint32_t dt = static_cast<uint32_t>(get_timestamp() - t_dp0);
                    if (moved && dt > max_pump) {
                        max_pump = dt;
                        max_pump_at = static_cast<uint32_t>(t_dp0);
                    }
                }
                return moved;
            };
        ...
        *phase = kPhasePoll;
        ...
        bool sweep_grew = false;
        {
            ...
            kernel_profiler::SpscZoneScope<kernel_profiler::DRISC_ZONE_SWEEP, SelfMarkNow> z_sweep(self_mark_now);
            {
                ...
                uint32_t n_ship = 0;
                // CV PIPELINED into the batch flights: only the first chunk of cores' tails are read and
                // waited serially -- enough to fill the first gather batches -- while the rest of the
                // grid's CV reads issue behind those batches' gathers and are scanned just-in-time when
                // the ship list runs dry. NO lambdas anywhere in this region: wrapping the scan in a
                // by-ref lambda cost 1-2% of sweep time in capture codegen alone, which the saturation
                // boundary amplifies ~200x -- the scan loop below must stay in this exact compilation
                // context.
                ...
                const uint32_t rd0 = NOC_STATUS_READ_REG(kReadNoc, NIU_MST_RD_RESP_RECEIVED);
                {
                    const uint64_t t_cv0 = kInstr != 0 ? get_timestamp() : 0;
                    {
                        kernel_profiler::SpscZoneScope<kernel_profiler::DRISC_ZONE_READ, SelfMarkPhase> z_cv(
                            self_mark_phase);
                        const uint64_t t_cvw0 = kSvcInstr != 0 ? get_timestamp() : 0;
                        // All cores' CV reads issue here, but the wait covers only the first chunk's
                        // RESPONSES (raw NIU counter delta): the rest land under the first batches'
                        // gathers. Responses can arrive out of order, so a counted-in response may
                        // belong to a later core -- then one chunk core scans last sweep's tails, which
                        // are stale but valid (monotonic): it under-ships and catches up next visit.
                        ...
                        invalidate_l1_cache();
                        if constexpr (kSvcInstr != 0) {
                            const uint32_t cdt = static_cast<uint32_t>(get_timestamp() - t_cvw0);
                            if (cdt > max_cvw) {
                                max_cvw = cdt;
                                max_cvw_at = static_cast<uint32_t>(t_cvw0);
                            }
                        }
                    }
                ...
                for (;;) {
                    ...
                    for (uint32_t c = scan_lo; c < scan_hi; c++) {
                        ...
                        // Deferral must be safe against ONE MORE service interval of production, and the
                        // level alone cannot say that: a core scanned just under the threshold at high rate
                        // blows the ring-fill margin two sweeps later -- the borderline tail of that was the
                        // knee flicker (2-10 stalls/run at any nonzero threshold; rotation- and hot-gating
                        // both measured insufficient). `grew` is the words produced in the LAST interval, so
                        // requiring it under the threshold too means a deferred core sits at most at
                        // ~2x threshold next visit -- while a trickle core still batches exactly as before.
                ...
                auto issue_core = [&](uint32_t c, uint32_t sl) {
                    ...
                    // The head payload and mirror advance are staged HERE, hidden behind the NIU
                    // acceptance of the same lane's gather read; after the batch barrier only the posted
                    // write itself remains on the release path. Safe because nothing reads the mirror
                    // between issue and that barrier, and the head leaves the core only after it.
                    ...
                    slot_hb[sl] = static_cast<uint8_t>(hb_slot);
                    ...
                    for (uint32_t r = 0; r < kNumRisc; r++) {
                        ...
                        uint32_t pad = 0;
                        if (take != 0) {
                            pad = kernel_profiler::spsc_span_pack_pad(
                                (start & (kRingWords - 1u)) + take > kRingWords ? 0u : start, off);
                            const uint32_t used = off - (kPrefix + kWireCtrl);
                            const uint32_t room =
                                kPayloadCapWords > used + pad ? kPayloadCapWords - used - pad : 0;
                            // A wrapping run occupies the WHOLE ring image in the slot, not its extent.
                            const uint32_t need =
                                (start & (kRingWords - 1u)) + take > kRingWords ? kRingWords : take;
                            if (need > room) {
                                take = 0;
                                pad = 0;
                            }
                        ...
                        if (hm + take > kRingWords) {
                            // A WRAPPING run ships as the whole ring image, ring-ordered, in ONE read;
                            // the decoder linearises by head (same wrap test on its side). At the
                            // boundary nearly every lane is near-full and wraps, and the split's
                            // second issue is pure loss on a sweep that is the DRISC's own
                            // instruction stream.
                ...
                auto advance_heads = [&](uint32_t n, uint32_t g) {
                    ...
                    for (uint32_t i = 0; i < n; i++) {
                        ...
                        const uint32_t sc = kHeadScratch + slot_hb[sl] * 32u;
                        ...
                        // at the knee. (Heads-on-NOC_INDEX re-measured 2026-08-30 under spool: no win --
                        // the pump owns that NoC under sustained load; there is no idle NoC here.)
                ...
                while (cur < n_ship) {
                    // Refill the ship list BEFORE it runs dry: the per-batch tail refresh below only
                    // covers cores already ON the list, so a batch issued right after a dry-out scan
                    // would consume sweep-start tails several microseconds stale -- at the saturation
                    // boundary that strands ~a hundred words per lane for a whole extra sweep.
                    ...
                    if (gen_shipped[gen]) {
                        ...
                        *phase = kPhBar1;
                        ...
                        if constexpr (kSvcInstr != 0) {
                            const uint32_t bdt = static_cast<uint32_t>(get_timestamp() - t_b0);
                            if (bdt > max_bar) {
                                max_bar = bdt;
                                max_bar_at = static_cast<uint32_t>(t_b0);
                            }
                        }
                        if (!flushed) {
                            egress_dead = true;
                    ...
                    const uint64_t t_rw0 = kSvcInstr != 0 ? get_timestamp() : 0;
                    {
                    ...
                    if constexpr (kSvcInstr != 0) {
                        const uint32_t rdt = static_cast<uint32_t>(get_timestamp() - t_rw0);
                        if (rdt > max_rdw) {
                            max_rdw = rdt;
                            max_rdw_at = static_cast<uint32_t>(t_rw0);
                        }
                    }
                    ...
                    // The rest of the grid's CV flew behind the batches above. Wait on the response
                    // COUNT, not a full barrier: gather responses also bump it, which only ever lets a
                    // scan see last sweep's tails -- stale but valid (monotonic), a benign under-ship.
        ...
        if (kInstr != 0 && win2_work && !win2_open) {
        ...
        // Busy sweeps below the pressure line skip the pump entirely: the spool IS the burst absorber,
        // and a capture that fits in it deserves pure gather -- one pump ship per sweep was ~15% of the
        // burst traffic leaving mid-burst and cost the ingress knee two delay steps. Idle sweeps and
        // pressure (the capture does not fit; the egress knee regime) pump exactly as before.
        if constexpr (kSpool) {
            if (pump_pressure || frames == frames_at_sweep_start) {
        ...
        // Freshness deadline, ONCE PER SWEEP and off the ship path (a per-batch timestamp read at the
        // saturation boundary measurably stalls producers): a gapless sub-pressure workload otherwise
        // never pumps until 5/8 -- seconds of host latency at low rates -- while a knee-regime burst
        // ends or reaches 5/8 well inside the deadline, so bounded freshness costs the burst knee
        // nothing. Engages the SAME hysteresis as the occupancy line; the ship block's 1/16 release
        // ends the episode, and holding the clock at `now` while draining restarts it from release.
        if constexpr (kSpool) {
            // Strided: even one wall-clock read per sweep measurably stalls producers at the
            // saturation boundary, and 64 sweeps (~1 ms) is noise against the 50 ms deadline.
            if (++fresh_tick >= 64u) {
                fresh_tick = 0;
                if (spool_wr == spool_rd) {
                    ...
                    if (spool_oldest == 0 || pump_pressure) {
                        spool_oldest = now;
    ...
    *phase = kPhTailBar;
    (void)write_barrier_bounded<false, kInstr != 0 ? kDoneAddr + 12u : 0u>(get_timestamp() + kCreditWaitCycles);
