...
// A dependent false for the unreachable else of an if constexpr chain -- over a type or
// over a value. The overloads are distinguished by template parameter kind: passing a
// value to the first (or a type to the second) is a deduction failure, not an error.
template <typename...>
