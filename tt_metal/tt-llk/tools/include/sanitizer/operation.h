...
struct OperationFpuEltwiseUnaryDatacopy : Operation<Exu::Fpu, Hoistable::Yes>
{
    ...
    // Blackhole's MOP depends on the face count, so its halves restate it; Wormhole's execute is
    // never handed it, and leaves the field unknown.
    struct NumFaces : Field<std::uint32_t>
    {
    };
...
// hence Hoistable::Yes, even though init does program per-op packer state.
struct OperationPack : Operation<Exu::Pack, Hoistable::Yes>
{
    template <typename T>
    using Field = StateField<OperationPack, T>;

    struct Untilize : Field<bool>
    {
    };
