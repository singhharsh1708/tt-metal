 ...
 *
 * Prints the site the recorded value was written from as well as the hook's own, so a reader has both
 * halves of the disagreement. A field the recorded state never saw prints as UNKNOWN, not as zero.
 * The silent zone is the caller's to honour; this reports whenever the values differ.
 *
 ...
 * @param exu The Exu's state, read for the seated status, the site it was seated at, the hook's own
 *        site, and the silent zone, which this one honours.
 * @param record The record the hook looked up, null when its Operation is not the seated one.
 * @return Whether the Operation is seated, so the caller can stop without testing the record again.
 */
