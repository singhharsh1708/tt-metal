...
// Aliased so there is one thread-id mapping, not two.
...
// Only Quasar's rendezvous still uses these, but they stay reserved everywhere: BARRIER_END anchors
...
__attribute__((always_inline)) inline void reset()
{
    ...
    *epoch_ptr = 0;
    ...
    // Each thread owns its slot. Required by the Quasar rendezvous: a relative generation needs a uniform start.
