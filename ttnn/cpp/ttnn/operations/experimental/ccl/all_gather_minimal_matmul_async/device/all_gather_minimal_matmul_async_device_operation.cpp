...
std::vector<ttnn::Tensor> all_gather_minimal_matmul_async(
    ...
    if (registry_mode != registry::Mode::Off) {
        ...
        // Request construction from live TT objects is a separate, still-unwired
        // seam. After cheap trace/explicit-override preflight, resolve() observes
        // the table and reports an incomplete request rather than guessing facts.
        registry_dispatch = registry::resolve_for_dispatch(registry_mode, std::nullopt, eligibility);
