...
bool tensor_contract_is_complete(const compact::TensorDescriptor& tensor) noexcept {
    if (tensor.rank == 0 || tensor.rank > compact::kMaxTensorRank) {
        return false;
    }
...
bool device_contract_has_required_facts(const compact::DeviceDescriptor& device) noexcept {
    return device.architecture == compact::kBlackholeArchitecture &&
...
bool request_has_required_facts(const compact::KeyDescriptor& key) noexcept {
    ...
    return key.schema_version == compact::kKeySchemaVersion && key.codegen_recipe_abi == compact::kCodegenRecipeAbi &&
           ...
           key.operation.output_tile_width != 0;
}
...
ResolutionReason compatibility_reason(const CompatibilityStatus status) noexcept {
    switch (status) {
        ...
        case CompatibilityStatus::DeviceMismatch: return ResolutionReason::CompatibilityMismatch;
    }
    return ResolutionReason::CompatibilityMismatch;
}
...
Resolution resolve_impl(
    ...
    const std::span<const compact::EntryDescriptor> entries) noexcept {
    if (mode == Mode::Off) {
    ...
    // Empty production storage is checked before request work so empty Shadow
    // and On remain cheap and behavior-preserving.
    if (entries.empty()) {
    ...
    const auto compatibility = validate_compatibility(lock, entries, request->key.device);
...
Resolution resolve(
    const Mode mode, const std::optional<RegistryRequest>& request, const Eligibility& eligibility) noexcept {
    return resolve_impl(mode, request, eligibility, generated::lock(), generated::entries());
}
...
DispatchResult resolve_for_dispatch(
    ...
    if (mode != Mode::Off) {
        resolution = resolver != nullptr ? resolver(mode, request, eligibility)
...
Resolution resolve_with_table_for_testing(
    ...
    const compact::TableLock& lock,
    const std::span<const compact::EntryDescriptor> entries) noexcept {
    return resolve_impl(mode, request, eligibility, lock, entries);
}
