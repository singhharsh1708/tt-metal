...
// The typed fields below are the complete tensor key. Opaque canonical
// memory-config / tensor-topology digests were removed for the same reason as
// the device attestation digests: their preimage is a runtime object no offline
// promoter can reproduce, so binding them made every entry unreachable. Buffer
// type, memory layout, dtype, layout, and the tile contract already separate
// the recipes that actually differ.
...
// Only portable, per-call device facts the running process can actually
// observe. Board capability class, ordered-mesh, physical-topology, and
// firmware/runtime-capability digests were attestation-only: no live provider
// could reproduce the promoter's bytes, so binding them made every entry
// permanently unreachable. They are canonical wildcards now; the live worker
// grid and mesh shape still select the certified cohort and bound candidate
// legality.
...
struct TableMetadata {
    ...
    // Inert provenance only: the AGMM source identity measured when the lock
...
// or exporter cannot silently preserve the same certified table identity. They
// are inert: readable in a diff and in telemetry, never a serving precondition.
struct TableLock {
...
inline constexpr TableValidationStatus validate_table_lock(
    ...
    if (certified_device.architecture != kBlackholeArchitecture || certified_device.device_count != kBh32DeviceCount ||
    ...
    // All remaining digests are emitter-computed provenance. They must be
    // present so a table's origin stays readable, but none of them is ever
    // compared against a live build, firmware, or source tree.
    if (sha256_is_zero(lock.metadata.content_sha256) || sha256_is_zero(lock.metadata.semantic_source_sha256) ||
        sha256_is_zero(lock.evidence_manifest_sha256) || sha256_is_zero(lock.predictor_sha256) ||
        sha256_is_zero(lock.exporter_sha256)) {
