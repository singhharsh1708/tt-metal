...
def test_geglu_op(device, dim, variant):
    ...
    # Accurate and FastLut both approximate the exact formulation, so
    # they share the same golden.
    golden = ttnn.get_golden_function(ttnn.geglu)(input_tensor, dim=dim, device=device, **geglu_kwargs)
    approximate = "tanh" if expected_variant == ttnn.GeluVariant.Tanh else "none"
    golden_expected = torch.nn.functional.gelu(B, approximate=approximate).unsqueeze(0).unsqueeze(0)

    assert torch.equal(golden, golden_expected), (
        f"geglu golden function ignored variant={expected_variant} for "
        f"{int((golden != golden_expected).sum().item())} of {golden.numel()} elements"
    )
