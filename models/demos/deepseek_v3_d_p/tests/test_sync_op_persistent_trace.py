...
The branches under test add `tokens_out` / `metadata_out` to
`inbound_socket_service_sync` so the caller owns the destination and the op
allocates nothing -- the precondition for capturing the call in a ttnn trace.
Neither branch ships a test that passes those arguments, so nothing in-tree
exercises the new code path, and nothing demonstrates the actual goal
(a trace containing the op).  These tests do both.
"""
...
# ---------------------------------------------------------------------------
# 8. The runner's REAL blocker: can the drain share ONE trace with the forward?
#
#    The traced forward reads slot_id and valid_global as two SEPARATE 1-element
#    device tensors (tt_prefill_block.py:631-632 -> zero_padded_kv_cache), while
#    the H2D drain produces ONE 3-word record.  Today the bridge is a host
#    readback + three copy_host_to_device_tensor writes
#    (tt_prefill_runtime.py:595-597), and a host->device write cannot live inside
#    a capture window -- which is what forces the drain OUT of the trace.
#
#    This asks whether the bridge can instead be done ON DEVICE, inside the
#    capture, with ttnn.slice into pre-allocated 1-element destinations.  If the
#    scattered scalars track the record written earlier in the SAME replay, the
#    circular dependency is broken with no C++ change.
# ---------------------------------------------------------------------------
@pytest.mark.parametrize(
    "mesh_device, device_params",
    [_params(trace_region_size=_TRACE_REGION)],
